...
# 获取游标
    ...
    def upThemeSingleAxis(self, list):
        sql = "INSERT INTO theme_single_axis VALUES(%s,%s,%s,%s,%s,%s,%s)"
...
def getlist(id):
    ...
    for node in themedata:
        ...
        if node['theme'] in themelist:
            datalist.append([node['author_id'],node['paper_id'],node['year'],node['theme'],MAXyear,MINyear,'-link-'.join(themelist)])
    return datalist
